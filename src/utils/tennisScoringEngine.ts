...
export function awardPoint(
  ...
  if (state.isTiebreak) {
    if (playerWon === 1) state.tiebreak_p1++;
  ...
  if (playerWon === 1) {
    ...
    else if (p1Pt === '30') state.gamePoint_p1 = '40';
    else if (p1Pt === '40') {
      if (p2Pt === '40') state.gamePoint_p1 = 'AD';
      else if (p2Pt === 'AD') state.gamePoint_p2 = '40';
      else gameWon = 1;
    } else if (p1Pt === 'AD') gameWon = 1;
  } else {
    if (p2Pt === '0') state.gamePoint_p2 = '15';
    else if (p2Pt === '15') state.gamePoint_p2 = '30';
    else if (p2Pt === '30') state.gamePoint_p2 = '40';
    else if (p2Pt === '40') {
      if (p1Pt === '40') state.gamePoint_p2 = 'AD';
      else if (p1Pt === 'AD') state.gamePoint_p1 = '40';
      else gameWon = 2;
    } else if (p2Pt === 'AD') gameWon = 2;
  }
  ...
  if (gameWon !== null) {
    ...
    if (state.currentSet === 1) {
      if (gameWon === 1) state.set1_p1++; else state.set1_p2++;
      p1Games = state.set1_p1; p2Games = state.set1_p2;
    } else if (state.currentSet === 2) {
      if (gameWon === 1) state.set2_p1++; else state.set2_p2++;
      p1Games = state.set2_p1; p2Games = state.set2_p2;
    } else if (state.currentSet === 3) {
      if (gameWon === 1) state.set3_p1++; else state.set3_p2++;
      p1Games = state.set3_p1; p2Games = state.set3_p2;
    }
...
export function checkMatchWinner(
  ...
  if ((state.set1_p1 >= target && state.set1_p1 - state.set1_p2 >= 2) || (state.set1_p1 === target + 1 && state.set1_p2 === target - 1) || (state.set1_p1 > state.set1_p2 && state.set1_p1 > tiebreakAt)) p1Sets++;
  else if ((state.set1_p2 >= target && state.set1_p2 - state.set1_p1 >= 2) || (state.set1_p2 === target + 1 && state.set1_p1 === target - 1) || (state.set1_p2 > state.set1_p1 && state.set1_p2 > tiebreakAt)) p2Sets++;

  if ((state.set2_p1 >= target && state.set2_p1 - state.set2_p2 >= 2) || (state.set2_p1 === target + 1 && state.set2_p2 === target - 1) || (state.set2_p1 > state.set2_p2 && state.set2_p1 > tiebreakAt)) p1Sets++;
  else if ((state.set2_p2 >= target && state.set2_p2 - state.set2_p1 >= 2) || (state.set2_p2 === target + 1 && state.set2_p1 === target - 1) || (state.set2_p2 > state.set2_p1 && state.set2_p2 > tiebreakAt)) p2Sets++;

  if (thirdSetMT.isMT) {
    if (state.set3_p1 >= thirdSetMT.target && state.set3_p1 - state.set3_p2 >= 2) p1Sets++;
    else if (state.set3_p2 >= thirdSetMT.target && state.set3_p2 - state.set3_p1 >= 2) p2Sets++;
  } else {
    if ((state.set3_p1 >= target && state.set3_p1 - state.set3_p2 >= 2) || (state.set3_p1 === target + 1 && state.set3_p2 === target - 1) || (state.set3_p1 > state.set3_p2 && state.set3_p1 > tiebreakAt)) p1Sets++;
    else if ((state.set3_p2 >= target && state.set3_p2 - state.set3_p1 >= 2) || (state.set3_p2 === target + 1 && state.set3_p1 === target - 1) || (state.set3_p2 > state.set3_p1 && state.set3_p2 > tiebreakAt)) p2Sets++;
  }
...
export function buildScoreString(s1_p1: number, s1_p2: number, s2_p1: number, s2_p2: number, s3_p1: number, s3_p2: number): string {
  const parts: string[] = [];
  if (s1_p1 > 0 || s1_p2 > 0 || s2_p1 > 0 || s2_p2 > 0 || s3_p1 > 0 || s3_p2 > 0) parts.push(`${s1_p1}/${s1_p2}`);
  if (s2_p1 > 0 || s2_p2 > 0 || s3_p1 > 0 || s3_p2 > 0) parts.push(`${s2_p1}/${s2_p2}`);
  if (s3_p1 > 0 || s3_p2 > 0) parts.push(`${s3_p1}/${s3_p2}`);
...
export function validateNormalSet(p1: number, p2: number): SetValidationResult {
  ...
  if ((p1 <= 5 && p2 <= 5) || (p1 === 6 && p2 === 5) || (p1 === 5 && p2 === 6) || (p1 === 6 && p2 === 6)) return { valid: true, isComplete: false, winner: null };
...
export function validateShortSet(p1: number, p2: number): SetValidationResult {
  if (p1 < 0 || p2 < 0) return { valid: false, isComplete: false, winner: null, error: 'Oyun sayıları negatif olamaz.' };
  if (p1 > 5 || p2 > 5) return { valid: false, isComplete: false, winner: null, error: 'Kısa sette bir oyuncu en fazla 5 oyun alabilir (5-3 veya 5-4).' };
  ...
  if ((p1 <= 3 && p2 <= 3) || (p1 === 4 && p2 === 3) || (p1 === 3 && p2 === 4) || (p1 === 4 && p2 === 4)) return { valid: true, isComplete: false, winner: null };
...
export function validateMatchTiebreak(p1: number, p2: number, target: number = 10): SetValidationResult {
  ...
  if (p1 >= target && p1 - p2 >= 2) {
    if (p1 > target && p1 - p2 > 2 && p2 < target - 1) return { valid: false, isComplete: false, winner: null, error: `Geçersiz Tie-Break skoru.` };
    return { valid: true, isComplete: true, winner: 1 };
  }
  if (p2 >= target && p2 - p1 >= 2) {
    if (p2 > target && p2 - p1 > 2 && p1 < target - 1) return { valid: false, isComplete: false, winner: null, error: `Geçersiz Tie-Break skoru.` };
...
export function validateFullMatchScores(s1_p1: number, s1_p2: number, s2_p1: number, s2_p2: number, s3_p1: number, s3_p2: number, format: string = '3 Normal Set', isFinishing: boolean = false): MatchValidationResult {
...
export function getValidPresetsForSet(setNumber: 1 | 2 | 3, format: string = '3 Normal Set'): Array<{ p1: number; p2: number; label: string }> {
  // Orijinal içerik (Yer kaplamaması için kodun devamı aynı kalabilir, sorunsuz çalışıyor)
  return [];
}
...
export function determineWinnerFromScores(p1Name: string, p2Name: string, s1_p1: number, s1_p2: number, s2_p1: number, s2_p2: number, s3_p1: number, s3_p2: number, format: string = '3 Normal Set'): { winner: string; p1Sets: number; p2Sets: number; isMatchFinished: boolean; winnerIndex: 1 | 2 | null } {
  const matchVal = validateFullMatchScores(s1_p1, s1_p2, s2_p1, s2_p2, s3_p1, s3_p2, format, false);
  let winner = 'Secilmedi';
  if (matchVal.winner === 1) winner = p1Name;
  else if (matchVal.winner === 2) winner = p2Name;
  else if (matchVal.p1SetsWon > matchVal.p2SetsWon) winner = p1Name;
  else if (matchVal.p2SetsWon > matchVal.p1SetsWon) winner = p2Name;
