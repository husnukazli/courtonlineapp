...
export const TennisDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [tournamentInfo, setTournamentInfo] = useState<{ ad: string; yer: string; tarih: string; not: string }>(() => {
    const saved = localStorage.getItem('co_tournament_info');
    if (saved) { try { return JSON.parse(saved); } catch { return { ad: '', yer: '', tarih: '', not: '' }; } }
