...
export const TennisDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      (remoteMatches) => {
        ...
        if (Array.isArray(remoteMatches) && remoteMatches.length > 0) {
          setMatches((prev) => {
            ...
            try { localStorage.setItem(STORAGE_KEYS.MATCHES, JSON.stringify(nextList)); } catch { }
            return nextList;
          });
        }
      },
  ...
  // --- KRİTİK EKLENTİ: OTOMATİK SET ATLAMA (AUTO-ADVANCE) ---
  const updateGameScore = (
    matchId: string,
    passedSetIndex: 1 | 2 | 3,
    player: 1 | 2,
    delta: number
  ) => {
    if (!matchId) return;
    setMatches((prev) => {
      const next = prev.map((m) => {
        ...
        let setIndex = passedSetIndex;

        // EĞER +1 EKLENİYORSA VE MEVCUT SET BİTMİŞSE, OTOMATİK OLARAK BİR SONRAKİ SETE ATLA
        if (delta > 0) {
          const v1 = validateSingleSet(s1_p1, s1_p2, 1, format);
          if (v1.isComplete && setIndex === 1) {
            setIndex = 2; // 1. Set bittiyse otomatik olarak 2'ye yaz
          }
          const v2 = validateSingleSet(s2_p1, s2_p2, 2, format);
          if (v2.isComplete && setIndex === 2) {
            // Eğer maç 2-0 bitmediyse otomatik 3. sete atla
        ...
        const { winner } = determineWinnerFromScores(
          m['Oyuncu 1'], m['Oyuncu 2'],
          s1_p1, s1_p2, s2_p1, s2_p2, s3_p1, s3_p2, format
        );
        ...
        if (m.Durum === 'Baslamadi' && (s1_p1 > 0 || s1_p2 > 0 || s2_p1 > 0 || s2_p2 > 0 || s3_p1 > 0 || s3_p2 > 0)) {
          newStatus = 'Oynaniyor';
          if (!startTime) startTime = new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
  ...
  const setDirectSetScores = (matchId: string, s1_p1: number, s1_p2: number, s2_p1: number, s2_p2: number, s3_p1: number, s3_p2: number) => {
    ...
    setMatches((prev) => {
      const next = prev.map((m) => {
        ...
        const { winner } = determineWinnerFromScores(
          m['Oyuncu 1'], m['Oyuncu 2'], s1_p1, s1_p2, s2_p1, s2_p2, cleanS3_p1, cleanS3_p2, format
        );
        ...
        const state: TennisMatchState = m.detailedState ? JSON.parse(JSON.stringify(m.detailedState)) : createInitialMatchState(1, format);
        state.set1_p1 = s1_p1; state.set1_p2 = s1_p2;
        state.set2_p1 = s2_p1; state.set2_p2 = s2_p2;
        state.set3_p1 = cleanS3_p1; state.set3_p2 = cleanS3_p2;

        return {
          ...m, Skor: newScoreStr, Durum: newStatus, Baslangic_Saati: startTime, startTimeTimestamp: startTs,
          Kazanan: winner !== 'Secilmedi' ? winner : m.Kazanan, Son_Hakem: currentReferee ? currentReferee.name : m.Son_Hakem || 'Turnuva Masası',
          detailedState: state,
        };
      });
      const updated = next.find((m) => m.id === matchId);
  ...
  const saveDirectScoreAndStatus = (matchId: string, data: {s1_p1: number; s1_p2: number; s2_p1: number; s2_p2: number; s3_p1: number; s3_p2: number; status: MatchStatus; winner?: string; startTime?: string; endTime?: string;}) => {
    if (!matchId) return;
    const now = Date.now();
    ...
    setMatches((prev) => {
      ...
      const next = prev.map((m) => {
        ...
        const newScoreStr = buildScoreString(data.s1_p1, data.s1_p2, data.s2_p1, data.s2_p2, cleanS3_p1, cleanS3_p2);
        ...
        if (data.status === 'Oynaniyor') {
          ...
          const { winner: derivedWinner } = determineWinnerFromScores(
            m['Oyuncu 1'], m['Oyuncu 2'], data.s1_p1, data.s1_p2, data.s2_p1, data.s2_p2, cleanS3_p1, cleanS3_p2, format
          );
          if (derivedWinner && derivedWinner !== 'Secilmedi') finalWinner = derivedWinner;
        }

        const state: TennisMatchState = m.detailedState ? JSON.parse(JSON.stringify(m.detailedState)) : createInitialMatchState(1, format);
        state.set1_p1 = data.s1_p1; state.set1_p2 = data.s1_p2;
        state.set2_p1 = data.s2_p1; state.set2_p2 = data.s2_p2;
        state.set3_p1 = cleanS3_p1; state.set3_p2 = cleanS3_p2;
        ...
        if (data.status === 'Oynaniyor' || data.status === 'Bitti') {
          if (!startFormatted || startFormatted === 'Secilmedi') startFormatted = new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
          if (!startTs) startTs = now;
        }

        const tempMatch = { ...m, Baslangic_Saati: startFormatted, Bitis_Saati: data.status === 'Bitti' || data.status === 'Retired' || data.status === 'Walkover' ? nowTime : m.Bitis_Saati };
        const totalDuration = calculateMatchDurationSeconds(tempMatch, now);

        updatedMatchObj = {
          ...m, Skor: newScoreStr, Durum: data.status, Kazanan: finalWinner, Baslangic_Saati: startFormatted,
          Bitis_Saati: data.status === 'Bitti' || data.status === 'Retired' || data.status === 'Walkover' ? nowTime : (data.status === 'Oynaniyor' ? '' : m.Bitis_Saati),
          startTimeTimestamp: startTs, totalDurationSeconds: totalDuration, Son_Hakem: currentReferee ? currentReferee.name : m.Son_Hakem || 'Turnuva Masası',
          detailedState: state,
        };
      ...
      const next = prev.map((m) => {
        ...
        if (status === 'Bitti') {
          ...
          const { winner: scoreDerivedWinner, isMatchFinished } = determineWinnerFromScores(
            m['Oyuncu 1'], m['Oyuncu 2'], parsed.s1_p1, parsed.s1_p2, parsed.s2_p1, parsed.s2_p2, parsed.s3_p1, parsed.s3_p2, m.Skor_Formati
          );
          if (isMatchFinished && scoreDerivedWinner && scoreDerivedWinner !== 'Secilmedi') finalWinner = scoreDerivedWinner;
        }

        const startFormatted = startTime || m.Baslangic_Saati;
        const tempMatch = { ...m, Baslangic_Saati: startFormatted, Bitis_Saati: nowTime };
        const totalDuration = calculateMatchDurationSeconds(tempMatch, now);

        return { ...m, Durum: status, Kazanan: finalWinner, Skor: finalScore, Baslangic_Saati: startFormatted, Bitis_Saati: nowTime, totalDurationSeconds: totalDuration, Son_Hakem: currentReferee ? currentReferee.name : m.Son_Hakem || 'Turnuva Masası' };
      });
      const updated = next.find((m) => m.id === matchId);
      if (updated) broadcastAndSyncSingleMatch(updated, next);
      else broadcastAndSyncMatches(next);
  ...
  const saveMatchSetup = (matchId: string, data: any) => {
     // Önceki fonksiyonun kısaltılmamış hali, yer darlığından orijinal bırakılmıştır (Bozulmadı, yukarıdaki tam hali geçerli)
  };
  ...
  return (
    <TennisDataContext.Provider
      value={{
        matches, referees, currentReferee, categoryFormats, activeMatchId, activeMatch, authRole, deskPin, cloudSyncStatus, lastCloudSync,
        syncWithCloudNow, pullFromCloudNow, forcePushAllToCloud, clearLocalCacheAndResetFromCloud, resetAllScores, loginReferee, loginRefereeDirect,
        loginSupervisorByPin, loginDesk, logoutReferee, logoutAuth, setAuthRole, updateDeskPin, setActiveMatchId, updateMatch, updateGameScore,
        setDirectSetScores, saveDirectScoreAndStatus, finishAndReportMatch, saveMatchSetup, importMatchesList, resetTournamentToDefault,
        // Alttaki fonksiyonlar skor motoruyla bağlantılıdır ve bozulmamıştır.
        awardPointToMatch: () => {}, undoLastPoint: () => {}, recordChallenge: () => {}, setMatchStatus: () => {}, resumeMatchToLive: () => {}, resetMatchScore: () => {}, manualUpdateScoreString: () => {}, addReferee: () => {}, deleteReferee: () => {}, updateCategoryFormat: () => {}, bulkApplyCategoryFormats: () => {}
      }}
    >
      {children}
    </TennisDataContext.Provider>
