...
interface QuickScoreEditModalProps {
 ...
 isOpen: boolean;
 onClose: () => void;
 onOpenSetup?: (match: MatchItem) => void; // Kura ekranına güvenli geçiş tetikleyicisi
}
...
export const QuickScoreEditModal: React.FC<QuickScoreEditModalProps> = ({
 ...
 setErrorMessage(
 `Maç henüz tamamlanmamıştır! (Mevcut Skor: ${currentScorePreview || '0/0'}). Bir oyuncunun maçı normal kazanabilmesi için 2 seti kurallara uygun tamamlamış olması gerekir. Eğer maçı canlıya döndürmek istiyorsanız 'Oynanıyor' seçiniz.`
 );
 return;
 }
 if (scoreWinnerName && selectedWinner !== scoreWinnerName) {
 setErrorMessage(
 `Skor uyumsuzluğu! Girilen skora göre (${currentScorePreview}) maçı ${scoreWinnerName} kazanmıştır. ${selectedWinner} kazanan olarak kaydedilemez.`
 );
 return;
 }
 }
 ...
 <span className={`text-[11px] font-black px-2.5 py-0.5 rounded-full uppercase ${status === 'Oynaniyor' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'}`}>
 {status === 'Oynaniyor' ? 'CANLI' : status}
 </span>
 </div>
 <p className="text-xs text-slate-400">{match.Kategori} • <span className="text-lime-400 font-bold">{match.Skor_Formati}</span></p>
 </div>
 </div>
 <button onClick={onClose} className="p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition"><X className="w-6 h-6" /></button>
 </div>
 ...
 <span>Skoru düzeltip maçı tekrar canlıya alabilirsiniz.</span>
 </div>
 <button type="button" onClick={handleResumeLiveMode} className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs transition flex items-center gap-1.5 shadow-md active:scale-95"><Play className="w-3.5 h-3.5 fill-current" /><span>Maçı Canlıya Al</span></button>
 </div>
 )}

 <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center justify-between shadow-inner">
 <div className="space-y-1.5 flex-1 pr-2">
 <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-lime-400 shrink-0"></span><span className="font-black text-sm sm:text-base text-lime-300 truncate">{p1Name}</span></div>
 <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-cyan-400 shrink-0"></span><span className="font-black text-sm sm:text-base text-cyan-300 truncate">{p2Name}</span></div>
 </div>
 ...
 <span className="font-mono text-xl sm:text-2xl font-black text-white tracking-wider">{currentScorePreview || '0/0'}</span>
 </div>
 </div>

 {errorMessage && (
 <div className="p-3.5 rounded-2xl bg-rose-950/90 border border-rose-500/60 text-rose-200 text-xs flex items-start gap-2.5 shadow-lg">
 <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />
 <div className="space-y-1"><strong className="block font-bold text-rose-300">Skor Uyarısı:</strong><span>{errorMessage}</span></div>
 </div>
 )}
 ...
 <div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wider text-slate-300">1. Set Oyunları</span><span className="text-xs font-mono font-bold text-slate-400">Skor: <strong className="text-white">{s1_p1} - {s1_p2}</strong></span></div>
 ...
 <button type="button" onClick={() => handleScoreIncrease(1, 1)} className="w-9 h-9 rounded-xl bg-lime-400 hover:bg-lime-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 <div className="flex items-center justify-between bg-slate-900 px-3 py-2 rounded-xl border border-slate-800">
 ...
 <button type="button" onClick={() => handleScoreIncrease(1, 2)} className="w-9 h-9 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* 2. SET */}
 <div className="bg-slate-950/90 p-3.5 rounded-2xl border border-slate-800 space-y-2.5">
 <div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wider text-slate-300">2. Set Oyunları</span><span className="text-xs font-mono font-bold text-slate-400">Skor: <strong className="text-white">{s2_p1} - {s2_p2}</strong></span></div>
 <div className="grid grid-cols-2 gap-3 items-center">
 <div className="flex items-center justify-between bg-slate-900 px-3 py-2 rounded-xl border border-slate-800">
 <span className="text-xs text-lime-400 font-bold truncate mr-2">{p1Name.split(' ')[0]}</span>
 <div className="flex items-center gap-2">
 ...
 <button type="button" onClick={() => handleScoreIncrease(2, 1)} className="w-9 h-9 rounded-xl bg-lime-400 hover:bg-lime-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 <div className="flex items-center justify-between bg-slate-900 px-3 py-2 rounded-xl border border-slate-800">
 <span className="text-xs text-cyan-400 font-bold truncate mr-2">{p2Name.split(' ')[0]}</span>
 ...
 <button type="button" onClick={() => handleScoreIncrease(2, 2)} className="w-9 h-9 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* 3. SET */}
 <div className="bg-slate-950/90 p-3.5 rounded-2xl border border-slate-800 space-y-2.5">
 <div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wider text-slate-300">3. Set / Karar Seti</span><span className="text-xs font-mono font-bold text-slate-400">Skor: <strong className="text-white">{s3_p1} - {s3_p2}</strong></span></div>
 <div className="grid grid-cols-2 gap-3 items-center">
 <div className="flex items-center justify-between bg-slate-900 px-3 py-2 rounded-xl border border-slate-800">
 <span className="text-xs text-lime-400 font-bold truncate mr-2">{p1Name.split(' ')[0]}</span>
 <div className="flex items-center gap-2">
 ...
 <button type="button" onClick={() => handleScoreIncrease(3, 1)} className="w-9 h-9 rounded-xl bg-lime-400 hover:bg-lime-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 <div className="flex items-center justify-between bg-slate-900 px-3 py-2 rounded-xl border border-slate-800">
 <span className="text-xs text-cyan-400 font-bold truncate mr-2">{p2Name.split(' ')[0]}</span>
 ...
 <button type="button" onClick={() => handleScoreIncrease(3, 2)} className="w-9 h-9 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 flex items-center justify-center font-black active:scale-95 transition"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 </div>
 </div>
 ...
 <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5"><Trophy className="w-3.5 h-3.5 text-lime-400" /><span>Maç Durumu:</span></span>
 <div className="flex items-center gap-1.5 bg-slate-900 p-1 rounded-xl border border-slate-800">
 {[
 { id: 'Oynaniyor' as MatchStatus, label: '▶️ Canlı' },
 ...
 setStatus(st.id);
 setErrorMessage('');
 if (st.id === 'Oynaniyor') { setSelectedWinner('Secilmedi'); setEndTime(''); }
 else if (st.id === 'Bitti') { if (!endTime) handleSetTimeNow('end'); if (scoreWinnerName) setSelectedWinner(scoreWinnerName); }
 }}
 className={`px-2.5 py-1 rounded-lg text-xs font-black transition ${status === st.id ? 'bg-lime-400 text-slate-950 shadow-md font-black' : 'text-slate-400 hover:text-white'}`}>{st.label}</button>
 ))}
 </div>
 </div>
 ...
 <div className="flex items-center justify-between"><label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Kazanan Oyuncu:</label>{status === 'Bitti' && scoreWinnerName && <span className="text-[11px] text-lime-400 font-bold flex items-center gap-1"><Lock className="w-3 h-3" /><span>Skorla Uyumlu</span></span>}</div>
 <div className="grid grid-cols-2 gap-2.5">
 <button type="button" onClick={() => handleSelectWinner(p1Name)} className={`p-3 rounded-2xl font-black text-xs sm:text-sm flex items-center justify-center gap-2 transition border ${selectedWinner === p1Name ? 'bg-lime-400 text-slate-950 border-lime-400' : 'bg-slate-900 text-lime-300 border-slate-800'}`}><Trophy className="w-4 h-4" /><span className="truncate">{p1Name.split(' ')[0]} Kazandı</span></button>
 <button type="button" onClick={() => handleSelectWinner(p2Name)} className={`p-3 rounded-2xl font-black text-xs sm:text-sm flex items-center justify-center gap-2 transition border ${selectedWinner === p2Name ? 'bg-cyan-400 text-slate-950 border-cyan-400' : 'bg-slate-900 text-cyan-300 border-slate-800'}`}><Trophy className="w-4 h-4" /><span className="truncate">{p2Name.split(' ')[0]} Kazandı</span></button>
 </div>
 </div>
 ) : (
 ...
 <div className="flex items-center justify-between text-xs"><span className="text-slate-300 font-bold flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-cyan-400" /><span>Bitiş Saati:</span></span><input type="text" value={endTime} onChange={(e) => setEndTime(e.target.value)} placeholder="15:30" className="w-20 px-2 py-1 bg-slate-900 border border-slate-700 rounded-lg text-center font-mono font-bold text-white text-xs" /></div>
 <div className="flex items-center gap-2">
 <button type="button" onClick={() => adjustTimeMinutes('end', -5)} className="flex-1 py-1.5 px-2 rounded-xl bg-slate-900 text-slate-300 text-xs font-bold flex items-center justify-center gap-1"><Minus className="w-3 h-3" /><span>-5 dk</span></button>
 <button type="button" onClick={() => adjustTimeMinutes('end', 5)} className="flex-1 py-1.5 px-2 rounded-xl bg-slate-900 text-slate-300 text-xs font-bold flex items-center justify-center gap-1"><Plus className="w-3 h-3" /><span>+5 dk</span></button>
 <button type="button" onClick={() => handleSetTimeNow('end')} className="py-1.5 px-3 rounded-xl bg-cyan-400/20 text-cyan-300 text-xs font-bold">Şimdi</button>
 </div>
 </div>
 )}
 </div>
 ...
 <button type="button" onClick={onClose} className="flex-1 py-3.5 rounded-2xl bg-slate-800 text-slate-300 font-bold text-xs sm:text-sm">İptal</button>
 <button type="button" onClick={handleSave} className="flex-2 py-3.5 px-5 rounded-2xl font-black text-sm shadow-xl flex items-center justify-center gap-2 bg-gradient-to-r from-lime-400 via-emerald-400 to-teal-400 text-slate-950"><Check className="w-5 h-5 stroke-[3]" /><span>Kaydet & Gönder</span></button>
 </div>
 </div>
 </div>
 );
