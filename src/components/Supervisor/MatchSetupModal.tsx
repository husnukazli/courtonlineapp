...
export const MatchSetupModal: React.FC<MatchSetupModalProps> = ({
  ...
  const [tbRule, setTbRule] = useState<'standard' | 'coman'>('standard');
  ...
  React.useEffect(() => {
    if (match) {
      ...
      setTbRule((match.tbKurali as any) || 'standard');
  ...
  const isDoubles = 
    (p1Name && p1Name.includes('/')) ||
    (p2Name && p2Name.includes('/')) ||
    (match.Kategori && match.Kategori.toLowerCase().includes('çift'));
  const p1Members = (p1Name || '').split('/').map((s) => s.trim()).filter(Boolean);
  const p2Members = (p2Name || '').split('/').map((s) => s.trim()).filter(Boolean);
  ...
  const handleJustSave = () => {
    ...
    saveMatchSetup(match.id, {
      ...
      tbType: tbRule,
      t1ServerIdx: isDoubles ? t1ServerIdx : undefined,
      t2ServerIdx: isDoubles ? t2ServerIdx : undefined,
      t1RecIdx: isDoubles ? t1RecIdx : undefined,
      t2RecIdx: isDoubles ? t2RecIdx : undefined,
    });
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-3 sm:p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in overflow-y-auto">
      ...
      <div className="bg-slate-900 border border-slate-700/50 rounded-3xl p-4 sm:p-6 w-full max-w-xl shadow-2xl space-y-4 my-auto relative">
        ...
        {/* ÇİFTLER MAÇI: SERVİS VE KARŞILAMA SIRASI */}
        {isDoubles && (p1Members.length > 1 || p2Members.length > 1) && (
          <div className="bg-slate-950/90 p-3 rounded-2xl border border-indigo-950/70 space-y-2.5 text-left">
            <div className="text-[10px] font-black text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
              <span>👥 Çiftler — 1. Set Servis & Karşılama Sırası</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {/* 1. Takım */}
              <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
                <span className="text-[10px] font-extrabold uppercase text-slate-300 block truncate">1. Takım ({p1Members.join(' / ')})</span>
                <div className="space-y-1.5">
                  <div>
                    <label className="text-[9px] font-semibold text-slate-400 block">İlk Servis:</label>
                    <select
                      value={t1ServerIdx}
                      onChange={(e) => setT1ServerIdx(Number(e.target.value) as 0 | 1)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white font-bold"
                    >
                      <option value={0}>{p1Members[0] || '1. Oyuncu'}</option>
                      <option value={1}>{p1Members[1] || '2. Oyuncu'}</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[9px] font-semibold text-slate-400 block">İlk Karşılayan (Sağ/Deuce):</label>
                    <select
                      value={t1RecIdx}
                      onChange={(e) => setT1RecIdx(Number(e.target.value) as 0 | 1)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white font-bold"
                    >
                      <option value={0}>{p1Members[0] || '1. Oyuncu'}</option>
                      <option value={1}>{p1Members[1] || '2. Oyuncu'}</option>
              ...
              {/* 2. Takım */}
              <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
                <span className="text-[10px] font-extrabold uppercase text-slate-300 block truncate">2. Takım ({p2Members.join(' / ')})</span>
                <div className="space-y-1.5">
                  <div>
                    <label className="text-[9px] font-semibold text-slate-400 block">İlk Servis:</label>
                    <select
                      value={t2ServerIdx}
                      onChange={(e) => setT2ServerIdx(Number(e.target.value) as 0 | 1)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white font-bold"
                    >
                      <option value={0}>{p2Members[0] || '1. Oyuncu'}</option>
                      <option value={1}>{p2Members[1] || '2. Oyuncu'}</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[9px] font-semibold text-slate-400 block">İlk Karşılayan (Sağ/Deuce):</label>
                    <select
                      value={t2RecIdx}
                      onChange={(e) => setT2RecIdx(Number(e.target.value) as 0 | 1)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white font-bold"
                    >
                      <option value={0}>{p2Members[0] || '1. Oyuncu'}</option>
                      <option value={1}>{p2Members[1] || '2. Oyuncu'}</option>
        ...
        {/* BAŞLANGIÇ SAATİ AYARI */}
        <div className="bg-slate-950/80 p-2.5 rounded-2xl border border-slate-800 flex items-center justify-between text-left">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="text-[11px] font-bold text-slate-300">Başlangıç Saati:</span>
            <input
              type="time"
              value={baslangicSaati}
              onChange={(e) => setBaslangicSaati(e.target.value)}
              className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white font-black w-24 text-center focus:border-emerald-500"
            />
          </div>
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="px-2 py-1 bg-slate-900 hover:bg-slate-800 border border-slate-700 rounded-lg text-[10px] font-bold text-slate-300"
            >
              -5 dk
            </button>
            <button
            ...
            <button
              ...
              className="px-2 py-1 bg-emerald-950/60 hover:bg-emerald-900/80 border border-emerald-600/50 rounded-lg text-[10px] font-bold text-emerald-300"
            >
              Şimdi
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1 text-left">
          ...
          <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800 space-y-2">
            <label className="text-[10px] font-extrabold uppercase text-slate-400 block">Maç Formatı</label>
            <select
              value={skorFormati}
          ...
          {/* Tie-Break Kuralı */}
          <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800 space-y-2">
            <label className="text-[10px] font-extrabold uppercase text-slate-400 block">Tie-Break Kuralı</label>
            <select
              value={tbRule}
              onChange={(e) => setTbRule(e.target.value as 'standard' | 'coman')}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl px-2 py-2 text-[10px] sm:text-xs text-white font-bold"
            >
              <option value="standard">Standart (Her 6 sayıda)</option>
              <option value="coman">Coman (1. sayıdan sonra her 4'te)</option>
            </select>
            <div className="text-[9px] text-slate-500 leading-tight">Skorbordda otomatik uygulanır.</div>
          </div>

          {/* Kort Değiştirme ve Taşıma Modülü */}
          <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800 space-y-2">
            ...
            <div className="text-[9px] text-slate-500 leading-tight">Seçtiğiniz korta sırasına göre taşınır.</div>
          </div>
        </div>
