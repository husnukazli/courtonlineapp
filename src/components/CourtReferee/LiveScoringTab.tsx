...
export const LiveScoringTab: React.FC<LiveScoringTabProps> = ({ onBackToSetup }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-12">
      ...
      <div className="bg-slate-900 border-2 border-emerald-700/60 rounded-3xl p-3 sm:p-5 shadow-2xl shadow-emerald-900/20 space-y-4">
      ...
      <div className="bg-emerald-950/30 border border-emerald-800/40 rounded-2xl p-2.5 space-y-1.5">
        ...
        <div className="grid grid-cols-5 gap-1.5 text-xs font-bold">
          {[{ id: 'NORMAL', label: 'Normal' }, { id: 'ACE', label: 'Ace ⚡' }, { id: 'WINNER', label: 'Winner 💥' }, { id: 'DOUBLE_FAULT', label: 'Çift Hata' }, { id: 'UNFORCED_ERROR', label: 'Hata ❌' }].map((pt) => (
            <button key={pt.id} type="button" onClick={() => setSelectedPointType(pt.id as any)} className={`py-2 px-1 rounded-xl text-[11px] border text-center transition ${selectedPointType === pt.id ? 'border-lime-400 bg-lime-400/20 text-lime-300 font-extrabold' : 'border-emerald-900/50 bg-emerald-950/30 text-emerald-400/50 hover:text-emerald-200'}`}>{pt.label}</button>
          ))}
        </div>
      </div>
