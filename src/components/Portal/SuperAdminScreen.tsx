...
export const SuperAdminScreen: React.FC<Props> = ({ onLogout }) => {
 ...
 // Başhakem formu
 const [baskahakemForm, setBaskahakemForm] = useState({ ad: '', sifre: '', tournamentId: '' });
 ...
 const [duzenleBaskahakemForm, setDuzenleBaskahakemForm] = useState({ ad: '', sifre: '', tournamentId: '' });
 ...
 setKayitYukleniyor(true);
 // createTournament fonksiyonu Firebase yapısında set/update işlemlerini de id parametresi veya nesne üzerinden yürütür.
 // Mevcut firebaseSync modelinizde spesifik update fonksiyonu yoksa bile createTournament dokümanı ezer/günceller.
 ...
 if (!baskahakemForm.ad.trim() || !baskahakemForm.sifre.trim() || !baskahakemForm.tournamentId) {
 ...
 { id: `bh_${Date.now()}`, ...baskahakemForm }
 ]
 };
 const ok = await saveSuperAdminConfig(yeniCfg);
 if (ok) {
 setConfig(yeniCfg);
 setBaskahakemForm({ ad: '', sifre: '', tournamentId: '' });
 ...
 if (!duzenleBaskahakemForm.ad.trim() || !duzenleBaskahakemForm.sifre.trim() || !duzenleBaskahakemForm.tournamentId) {
 flash('❌ Tüm alanları doldurun.'); return;
 }

 const guncelListe = (config?.bashakem_listesi || []).map(bh => {
 if (bh.id === duzenlenenBaskahakemId) {
 return { id: duzenlenenBaskahakemId, ...duzenleBaskahakemForm };
 }
 return bh;
 });
 ...
 <span className="font-black text-white">Süper Admin</span>
 </div>
 <div className="flex items-center gap-2">
 {msg && <span className="text-xs font-bold text-emerald-400">{msg}</span>}
 <button onClick={yukle} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition"><RefreshCw className="w-4 h-4" /></button>
 <button onClick={onLogout} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 text-xs transition">
 ...
 <h2 className="font-black text-lg text-white flex items-center gap-2"><Trophy className="w-5 h-5 text-lime-400" />Turnuvalar</h2>
 ...
 {yeniGosteriliyor && (
 <div className="bg-slate-900 border border-lime-500/30 rounded-2xl p-5 mb-4 space-y-3">
 <h3 className="font-bold text-sm text-lime-300">Yeni Turnuva</h3>
 {[
 ...
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-lime-400" />
 </div>
 ))}
 <div>
 <label className="text-xs font-bold text-slate-400 mb-1 block">Not</label>
 <textarea value={yeniForm.not} placeholder="İsteğe bağlı not..."
 onChange={e => setYeniForm(p => ({ ...p, not: e.target.value }))}
 rows={2} className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-lime-400 resize-none" />
 </div>
 ...
 className="px-4 py-2 bg-slate-800 text-slate-300 font-bold rounded-xl text-sm">İptal</button>
 </div>
 </div>
 )}
 ...
 {tournaments.map(t => (
 <div key={t.id} className="p-4 bg-slate-900 border border-slate-700/60 rounded-xl space-y-3">
 {duzenlenenTurnuvaId === t.id ? (
 <div className="space-y-3">
 <h4 className="text-xs font-bold text-lime-400">Turnuvayı Düzenle</h4>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Turnuva Adı *</label>
 <input type="text" value={duzenleTurnuvaForm.ad}
 onChange={e => setDuzenleTurnuvaForm(p => ({ ...p, ad: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-lime-400" />
 </div>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Lokasyon</label>
 <input type="text" value={duzenleTurnuvaForm.yer}
 onChange={e => setDuzenleTurnuvaForm(p => ({ ...p, yer: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-lime-400" />
 </div>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Tarih</label>
 <input type="text" value={duzenleTurnuvaForm.tarih}
 onChange={e => setDuzenleTurnuvaForm(p => ({ ...p, tarih: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-lime-400" />
 </div>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Not</label>
 ...
 className="px-3 py-1.5 bg-slate-800 text-slate-300 font-bold rounded-lg text-xs">İptal</button>
 </div>
 </div>
 ) : (
 <div className="flex items-center gap-3">
 <div className="flex-1 min-w-0">
 <div className="font-bold text-sm text-white">{t.ad}</div>
 <div className="text-xs text-slate-400 flex gap-3 mt-0.5">
 {t.yer && <span>📍 {t.yer}</span>}
 {t.tarih && <span>📅 {t.tarih}</span>}
 <span className="text-slate-600 font-mono text-[10px]">{t.id}</span>
 </div>
 </div>
 <div className="flex items-center gap-1 shrink-0">
 <button onClick={() => handleTurnuvaDuzenleAc(t)}
 className="p-2 text-cyan-400/80 hover:text-cyan-300 hover:bg-slate-800 rounded-lg transition">
 <Edit2 className="w-4 h-4" />
 </button>
 <button onClick={() => handleTurnuvaSil(t)}
 className="p-2 text-rose-400/60 hover:text-rose-300 hover:bg-rose-950/30 rounded-lg transition">
 ...
 {tournaments.length === 0 && <p className="text-slate-500 text-sm text-center py-4">Henüz turnuva yok.</p>}
 </div>
 </section>
 ...
 <section>
 <div className="flex items-center justify-between mb-4">
 <h2 className="font-black text-lg text-white flex items-center gap-2"><Shield className="w-5 h-5 text-cyan-400" />Başhakemler</h2>
 ...
 <div className="bg-slate-900 border border-cyan-500/30 rounded-2xl p-5 mb-4 space-y-3">
 <h3 className="font-bold text-sm text-cyan-300">Yeni Başhakem</h3>
 <div>
 <label className="text-xs font-bold text-slate-400 mb-1 block">Ad Soyad</label>
 ...
 <div>
 <label className="text-xs font-bold text-slate-400 mb-1 block">Şifre</label>
 <div className="relative">
 <input type={sifreGoster ? 'text' : 'password'} value={baskahakemForm.sifre} placeholder="Şifre belirleyin"
 onChange={e => setBaskahakemForm(p => ({ ...p, sifre: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-cyan-400 pr-10" />
 <button type="button" onClick={() => setSifreGoster(!sifreGoster)} className="absolute right-3 top-2.5 text-slate-500 hover:text-white">
 {sifreGoster ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
 </button>
 </div>
 </div>
 <div>
 <label className="text-xs font-bold text-slate-400 mb-1 block">Atanacak Turnuva</label>
 ...
 return (
 <div key={bh.id} className="p-4 bg-slate-900 border border-slate-700/60 rounded-xl space-y-3">
 {duzenlenenBaskahakemId === bh.id ? (
 <div className="space-y-3">
 <h4 className="text-xs font-bold text-cyan-400">Başhakemi Düzenle</h4>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Ad Soyad</label>
 <input type="text" value={duzenleBaskahakemForm.ad}
 onChange={e => setDuzenleBaskahakemForm(p => ({ ...p, ad: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-cyan-400" />
 </div>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Şifre</label>
 <div className="relative">
 <input type={duzenleSifreGoster ? 'text' : 'password'} value={duzenleBaskahakemForm.sifre}
 onChange={e => setDuzenleBaskahakemForm(p => ({ ...p, sifre: e.target.value }))}
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-cyan-400 pr-10" />
 <button type="button" onClick={() => setDuzenleSifreGoster(!duzenleSifreGoster)} className="absolute right-3 top-2.5 text-slate-500 hover:text-white">
 {duzenleSifreGoster ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
 </button>
 </div>
 </div>
 <div>
 <label className="text-xs text-slate-400 font-bold mb-1 block">Atanan Turnuva</label>
 ...
 <div className="font-bold text-sm text-white">{bh.ad}</div>
 <div className="text-xs text-slate-400">{turnuva?.ad || bh.tournamentId}</div>
 </div>
 <div className="flex items-center gap-1 shrink-0">
 ...
 {(config?.bashakem_listesi || []).length === 0 && <p className="text-slate-500 text-sm text-center py-4">Henüz başhakem eklenmedi.</p>}
 </div>
 </section>

 {/* Süper Admin Şifre */}
 <section className="bg-slate-900 border border-slate-700/60 rounded-2xl p-5 space-y-3 max-w-sm">
 <h3 className="font-bold text-sm text-white">Süper Admin Şifresini Değiştir</h3>
 <div className="relative">
 ...
 className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-slate-500 pr-10" />
 <button type="button" onClick={() => setSuperSifreGoster(!superSifreGoster)} className="absolute right-3 top-2.5 text-slate-500 hover:text-white">
