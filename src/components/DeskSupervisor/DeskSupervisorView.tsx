...
export const DeskSupervisorView: React.FC = () => {
 ...
 if (confirm('KRİTİK UYARI: Telefondaki önbellek, yerel hafıza ve buluttaki (Firestore) TÜM AKTİF MAÇLAR tamamen silinecektir. Bu işlem geri alınamaz! Yeni günün programını yüklemeden önce her şeyi sıfırlamak istediğinize emin misiniz?')) {
 setIsSyncingAction(true);
 setImportMsg('🔄 Bulut veritabanı ve yerel önbellek kökten temizleniyor...');
 const success = await clearLocalCacheAndResetFromCloud();
 setIsSyncingAction(false);
 if (success) {
 setImportMsg('✨ Tüm turnuva geçmişi ve bulut başarıyla temizlendi! Artık yeni günün JSON verisini yükleyebilirsiniz.');
 } else {
 setImportMsg('❌ Sıfırlama esnasında veritabanı hatası oluştu.');
 }
 ...
 <button type="button" disabled={isSyncingAction} onClick={handleAbsoluteReset} className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-extrabold text-xs transition shadow flex items-center gap-2">
 <Trash2 className="w-4 h-4" />
 <span>Tüm Turnuvayı ve Bulutu Sıfırla</span>
 </button>
